package com.treasure.recycler_view.widget;
...
//        mRightBitmap = BitmapFactory.decodeResource(this.getContext().getResources(), R.drawable.fengshan);
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (attr) {
                ...
                case R.styleable.leafView_circleLeftStyle:
                    ...
                    break;
                case R.styleable.leafView_borderWidth1:
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawArc(mRect,90,180,true,paint);
        canvas.drawRect(mRectCen,paint);

        /***********seek****************/
        paint.reset();
        paint.setAntiAlias(true);
        paint.setColor(mSeekColor);
        ...
        if(mCurrentProgressPosition<=mHeight-mBorderWidth){
            ...
            //进度的seekbar
            canvas.drawArc(mRectSeek,mStartAngle,mAngle*2,mCircleStyle,paint);
        }else { //判断在圆弧外了,需要填满圆弧,并且还要填矩形
            canvas.drawArc(mRectSeek,90,180,mCircleStyle,paint);
            seekRect=new Rect(mHeight/2,mBorderWidth,mCurrentProgressPosition-mHeight/2,mHeight-mBorderWidth);
            canvas.drawRect(seekRect,paint);
        }
        /***********seek over****************/
        //内层弧度
        ...
        canvas.drawArc(mRect,90,180,false,paint);
        //上下两条线
        ...
        canvas.drawCircle(mWidth-mHeight/2,mHeight/2,mHeight/2-mCircleWidth/2,paint);
        //画右边的图片
//        paint.reset();
//        canvas.drawBitmap(mRightBitmap,mWidth-mHeight+mBorderWidth,mBorderWidth,paint);
    }
